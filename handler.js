  ...
  user.commandCount = 1
               ...
               if (plugin.rowner && plugin.owner && !(isROwner || isOwner)) { // Both Owner
                ...
                if (!isPrems && plugin.money && global.db.data.users[m.sender].money < plugin.money * 1) {
                    this.reply(m.chat, `⚜️ 𝙉𝙊 𝙏𝙄𝙀𝙉𝙀 𝗪𝗜𝗧𝗧𝗬𝗖𝗢𝗜𝗡𝗦`, m)
                    continue     
		}
		    ...
		    continue // If the level has not been reached
                ...
                try {
                 ...
                 if (m.money)
                        m.reply(+m.money + ' 𝗪𝗜𝗧𝗧𝗬𝗖𝗢𝗜𝗡𝗦 ⚜️ 𝙐𝙎𝘼𝘿𝙊(𝙎)')
...
text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Welcome, @user!').replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || '🥵 𝗦𝘂𝗽𝗲𝗿 𝗪𝗶𝘁𝘁𝘆-𝗕𝗼𝘁 🥵') :
...
let prep = generateWAMessageFromContent(m.chat, { extendedTextMessage: { text: msg, contextInfo: { externalAdReply: { title: lenguajeGB.smsAvisoAG().slice(0,-2), body: [wm, '🥵 𝗦𝘂𝗽𝗲𝗿 ' + wt + ' 🥵', '🌟 https://instagram.com/gabrielacosta_15'].getRandom(), thumbnail: wittyImg.getRandom(), sourceUrl: [md, nna, ig, fb].getRandom() }}}}, tg)
